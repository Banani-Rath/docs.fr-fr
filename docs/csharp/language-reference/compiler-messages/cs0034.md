...
L'opérateur 'opérateur' est ambigu pour des opérandes de type 'type1' et 'type2'  
...
```  
 ...
 En C\# 1.1, un bogue dans le compilateur permettait de définir une classe possédant des conversions implicites définies par l'utilisateur en `int` et `bool`, et d'utiliser un opérateur de bits `AND` \(`&`\) sur les objets de ce type.  Dans C\# 2.0, ce bogue a été corrigé pour mettre le compilateur en conformité avec la spécification C\#. Par conséquent, le code suivant générera désormais l'erreur CS0034 :  
