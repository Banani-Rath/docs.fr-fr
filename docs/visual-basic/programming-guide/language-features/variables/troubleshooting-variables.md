...
Cette page répertorie quelques problèmes courants qui peuvent se produire quand vous utilisez des variables en [!INCLUDE[vbprvb](../../../../csharp/programming-guide/concepts/linq/includes/vbprvb_md.md)].  
...
## D’autres composants ne peuvent pas accéder à votre variable  
 Les noms en [!INCLUDE[vbprvb](../../../../csharp/programming-guide/concepts/linq/includes/vbprvb_md.md)]*ne respectent pas la casse*. Si deux noms diffèrent uniquement par la casse, le compilateur les interprète comme étant identiques. Par exemple, il considère que `ABC` et `abc` font référence au même élément déclaré.  
