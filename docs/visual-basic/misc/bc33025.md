...
Un opérateur de conversion est déclaré avec un type d’interface comme type de retour.  
 ...
 Au moment de la compilation, [!INCLUDE[vbprvb](../../csharp/programming-guide/concepts/linq/includes/vbprvb_md.md)] considère qu’une conversion prédéfinie existe dans tous les types de référence à une interface. Une telle conversion risque d’échouer au moment de l’exécution, mais le compilateur ne peut pas prédire les résultats de l’exécution, donc il permet la compilation d’une telle conversion.  
