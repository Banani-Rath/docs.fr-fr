...
Une procédure générique est appelée sans fournir d’arguments de type, et les arguments de type déduits produisent un ou plusieurs avertissements.  
 ...
 Une *contrainte* sur un type de paramètre limite les arguments de type qui peuvent lui être passés. Par exemple, un paramètre de type peut être contraint à être une classe qui implémente l’interface <xref:System.IComparable%601>. Pour plus d’informations, consultez la section « Contraintes » dans [Generic Procedures in Visual Basic](../../visual-basic/programming-guide/language-features/data-types/generic-procedures.md).  
