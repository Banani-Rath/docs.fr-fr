...
L’outil ALink, appelé par le compilateur [!INCLUDE[vbprvb](../../csharp/programming-guide/concepts/linq/includes/vbprvb_md.md)], signale une erreur lors de la génération de l’assembly. Il est possible qu’un assembly signé fasse référence à un assembly non signé.  
  
 Ce message est un avertissement. Le compilateur continue de générer l’assembly. Pour plus d’informations sur le masquage des avertissements ou le traitement des avertissements en tant qu’erreurs, consultez [Configuration d'avertissements en Visual Basic](/visual-studio/ide/configuring-warnings-in-visual-basic).  
